...
1.  **Ingestion**: Subscribes to configured RSS/Atom feeds (e.g., Socket.dev, Phylum, OSS-Security) to fetch latest articles. It uses ETag and Last-Modified headers to efficiently poll for new content.
...
3.  **Extraction**: If a compromise is detected, the LLM extracts key details: affected package name, ecosystem, version range, and compromised timeframe.
4.  **Deduplication**: Checks a local database to ensure the compromise hasn't been reported before.
5.  **Impact Assessment**: Uses `osc` (openSUSE Commander) to search for the affected package in OBS. If found, it analyzes changelogs and source listings to determine if the package was updated during the compromised window or contains known malicious files.
...
-   `sources`: List of RSS/Atom feeds to monitor.
